...
def playGame(wordList):
    ...
    while True:
        ...
        if user_input not in ['n', 'r']:
            print('Invalid command.')
            continue
        
        # If 'r' is chosen but no hand has been played yet
        if user_input == 'r' and hand is None:
            print("You have not played a hand yet. Please play a new hand first!")
            continue
        
        # Ask who should play the hand
        while True:
            user_input2 = input('Enter u to have yourself play, c to have the computer play: ')
            if user_input2 not in ['u', 'c']:
                print('Invalid command.')
            else:
                break
